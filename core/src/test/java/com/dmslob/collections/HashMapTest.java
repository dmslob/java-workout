...
public class HashMapTest {
	...
	@Test
	public void should_throw_IllegalStateException_on_toMap() {
		// given
		record Employee(String name, Integer age, BigDecimal salary) {
		}
		var e1 = new Employee("Alice", 30, BigDecimal.valueOf(1000));
		var e2 = new Employee("Alice", 16, BigDecimal.valueOf(2000));
		var employees = List.of(e1, e2);
		// when
		// then
		assertThatThrownBy(() -> employees.stream()
				.collect(Collectors.toMap(Employee::name, Employee::salary)))
				.isInstanceOf(IllegalStateException.class)
				.hasMessageContaining("Duplicate key Alice (attempted merging values 1000 and 2000)");
	}

	@Test
	public void should_throw_NPE_on_toMap() {
		// given
		record Employee(String name, Integer age, BigDecimal salary) {
		}
		var e1 = new Employee("Alice", 30, null);
		var e2 = new Employee("Alice", 16, BigDecimal.valueOf(2000));
		var employees = List.of(e1, e2);
		// when
		// then
