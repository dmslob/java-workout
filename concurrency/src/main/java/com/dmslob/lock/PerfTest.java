...
public class PerfTest {
    ...
    private static final Spaceship[] SPACESHIPS = {new SynchronizedSpaceship(), new ReadWriteLockSpaceShip(), new ReentrantLockSpaceship(),
            new StampedLockSpaceship(), new StampedLockWithRetriesSpaceship(), new LockFreeSpaceship(),};
    ...
    private static void perfRun(final Spaceship spaceship) throws Exception {
        ...
        System.out.format("%d readers %d writers %22s %s%n", NUM_READERS, NUM_WRITERS, spaceship.getClass().getSimpleName(), results);
    }
    ...
    static class Results {
        ...
        public String toString() {
            ...
            return "reads=" + readsSummary + Arrays.toString(reads) + " moves=" + movesSummary + Arrays.toString(moves) + " readAttempts=" + Arrays
                    .toString(readAttempts) + " moveAttempts=" + Arrays.toString(moveAttempts) + " observedMoves=" + Arrays.toString(observedMoves);
        }
    }
    ...
    static class WriterRunner implements Runnable {
        ...
        WriterRunner(final int id, final Results results, final Spaceship spaceship, final AtomicBoolean runningFlag, final CyclicBarrier barrier,
    ...
    static class ReaderRunner implements Runnable {
        ...
        ReaderRunner(final int id, final Results results, final Spaceship spaceship, final AtomicBoolean runningFlag, final CyclicBarrier barrier,
                     final CountDownLatch latch) {
            this.id = id;
            this.results = results;
            this.spaceship = spaceship;
