package com.dmslob.asynchronous.v1;

import com.dmslob.asynchronous.ExchangeService;
...
public class BestPriceFinder {
    ...
    public List<String> findPricesInUSD3(String product) {
        ...
        Stream<CompletableFuture<String>> priceFuturesStream = shops.stream().map(shop -> CompletableFuture.supplyAsync(() -> shop.getPrice(product))
                .thenCombine(CompletableFuture.supplyAsync(() -> ExchangeService.getRate(ExchangeService.Money.EUR, ExchangeService.Money.USD)), (price, rate) -> price * rate)
                .thenApply(price -> shop.getName() + " price is " + price));
